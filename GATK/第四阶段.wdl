...
public static void finalizeRecalibrationTables( final RecalibrationTables tables ) }{
	// RecalDatum:重矫正数据是单独的一个部分，每一部分统计了组合协变量的观察数量和reference错配的数量，
	...
	// if (rgDatum == null){
		...
		/// 在byReadGroup表中利用现存的datum组合qual datum
		// 该对象中加入其他对象中的所有数据，通过两个reported qualities，应用所expected的错误率，更新reported quality，输入：需要组合的RecalDatum
		// rgDatum.combine(qualDatum);
		// public void combine(final RecalDatum other) {
			// 在datum中，计算给定估计报告质量和观测值的预期误差数量:返回：对错误数量的正(可能是部分)估计
	...
	// private static void roundTableValues(final RecalibrationTables rt) {
		// 把double数值四舍五入到给定的小数点位。举例:四舍五入3.1415926到3位有效位位3.142.要求是:该方法求解要准确。写入和读取数字都是以字符串的形式表示。
