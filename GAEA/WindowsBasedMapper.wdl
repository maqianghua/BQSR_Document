...
protected void setup(Context context) throws IOException, InterruptedException{
	...
	/ 遍历ReadGroup List对象，并将其第i个元素SAMReadGroupRecord的SM(sampleName)标签值作为key，i作为值，插入到sampleIDs的HashMap对象中
	/ 获取SAM_RECORD_FILTER对应的class文件名
	/ 假如SAM_RECORD_FILTER对应的class文件名为null{
		/ 返回一个默认的SAMRecordFilter对象作为recordFilter
	}否则{
		/ 通过SAM_RECORD_FILTER对应的class文件名获取其字节码文件并建立SAMRecordFilter实例对象
	}
	/ 假如REFERENCE_REGION不为null{
		/ 获取region对象
		/ 通过REFERENCE_REGION利用region解析来自HDFS上的bed文件
		/ 通过REFERENCE_REGION利用regionExtend解析来自HDFS上的bed文件(包含延伸窗口)
	}
}
protected void map(LongWritable key, SamRecordWritable value, Context context) throws IOException, InterruptedException {
	/ 获取一行read：sam
	/ 通过regionExtend过滤read？（没有做过滤）
	...
	/ 根据read的比对起始位置，终止位置，染色体chrName的序列长度，获取扩展的窗体ID int[] wimNums
	...
	protected int[] getExtendPosition(int start, int end, int length){
		...
		/ winNum[1]:条件：read比对的起始位置>windowsExtendSize=500；是：(read比对的起始位置-windowsExtendSize)/windowsSize;否：0/windowsSize
		/ winNum[2]:条件：(read比对的终止位置+windowsExtendSize)>染色体长度；是：染色体长度/windowsSize;否:(read比对的终止位置+windowsExtendSize)/windowsSize;
	}
	/ 遍历winNum的值，将相同窗体read输出，相同的read不同的窗体也会输出{
		/ 设置keyout的组成元素
		/ setKey(sam, winNums[i]);
		protected void setKey(SAMRecord sam, int winNum) {
			/ 设置输出的键WindowsBasedWritable keyout由sampleID,染色体ID(有可能值为-1,没有比对到任何一个染色体上),窗体ID(从0开始计数的)，read比对在reference上的起始位置。
			...
			/ 设置keyout的组成元素(样本ID-sampleID，染色体索引chrIndex，窗体ID-winNum，read的比对到reference上的起始位置)
		}
